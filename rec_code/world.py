...
all_models  = ['mf', 'lgn', 'colakg', 'colakg_useragg', 'colakg_useragg_ssl', 'colakg_useragg_att', 'colakg_useragg_gate']
...
config['user_max_items'] = args.user_max_items


GPU = torch.cuda.is_available()
device = torch.device('cuda' if GPU else "cpu")
CORES = multiprocessing.cpu_count() - 4  # 保留4个核心给系统
seed = args.seed

# 设置PyTorch线程数
torch.set_num_threads(CORES)
torch.set_num_interop_threads(max(4, CORES//4))
